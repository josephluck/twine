  ...
  let { state, actions } = twine<any, any>(
    {
      ...
      computed(localState) {
        ...
        t.equal(
          localState.title,
          'not set',
          'computed state recieved correct local state in top level model',
        )
        return {
          foo: 'foo',
        }
        ...
        anotherModel: {
          ...
          computed(localState) {
            ...
            t.equal(
              localState.abc,
              123,
              'computed state recieved correct local state in child model',
            )
            return {
...
test('twine / computed / computed functions in parent models trigger when child models change state', function (
  t,
) {
  t.plan(12)
  let parentCounter = -1
